...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!companies) console.error("Error parsing JSON");
    else console.log("companies", companies); // Now 'data' is a JavaScript array object
    ...
    // Function to output company cards
    function outputCompanyCards() {
        companies.forEach(company => {
            const companyCard = new CompanyCard(company);
            section.innerHTML += companyCard.outputCard();
        });
    }
    ...
    function CompanyCard(company) {
        ...
        this.marketCap50 = company.stats.totalAssets;
        this.marketCap200 = company.stats.totalLiabilities;
        this.equity = company.stats.totalAssets - company.stats.totalLiabilities;
        this.tags = company.tags;

        // Currency formatter
        this.currency = (num) => new Intl.NumberFormat('en-US', {
            style: 'currency',
            currency: 'USD'
        }).format(num);

        // Billions formatter
        this.billions = (num) => new Intl.NumberFormat('en-US', {
            notation: "compact",
        ...
        this.outputCard = () => {
            ...
            cardHtml += `<h2>${this.symbol} - ${this.name}</h2>`;
            cardHtml += `<div>`;
            cardHtml += `<p>Share Price (50day avg): <span>${this.currency(this.day50)}</span></p>`;
            cardHtml += `<p>Share Price (200day avg): <span>${this.currency(this.day200)}</span></p>`;
            cardHtml += `<p>Market Cap (50day avg): <span>${this.billions(this.marketCap50)}</span></p>`;
            cardHtml += `<p>Market Cap (200day avg): <span>${this.billions(this.marketCap200)}</span></p>`;
            cardHtml += `<p>Net Revenue: <span>${this.currency(this.revenue)}</span></p>`;
            cardHtml += `<p>Shareholder Equity: <span>${this.currency(this.equity)}</span></p>`;
            cardHtml += `</div>`;
            cardHtml += `<footer>`;
            this.tags.forEach(tag => {
                cardHtml += `<small>${tag}</small>`;
            });
            cardHtml += `</footer>`;
            cardHtml += `</article>`;
            return cardHtml;
